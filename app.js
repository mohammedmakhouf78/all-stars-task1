...
async function translatePlease(childNodes, words){
    ...
    for (const childNode of childNodes) {
        ...
        // if(text.indexOf('.com') != -1 || text.indexOf('.AI') != -1 || text.indexOf('.AI') != -1)
        // {
        //     words[text] = text;
        //     childNode.textContent = words[text]
        //     continue;   
...
async function translateAll(){
    ...
    for (const fileName of Files)
    {
        ...
        getAllChildNodes(dom.window.document.querySelector('html'), childNodes)
        console.log(`there are ${childNodes.length} sentenses`);

        await translatePlease(childNodes, words)
...
function getAllChildNodes(element, childNodes){
    element.childNodes.forEach(childNode => {
        if (childNode.nodeName == "SCRIPT" || childNode.nodeName == "STYLE") {
            return;
        }
